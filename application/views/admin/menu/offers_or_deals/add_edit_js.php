<script type="text/javascript">
    ...
    $(document).on('click','#editDealsFormButtonConfirm',function() {
        dealsFormValidate();
        var isValid = $('#dealsEditForm').valid();
        if (isValid) {
            $('#dealsEditForm').submit();
        }
    });

    function isAddItemBtnDisabled(callback) {
        $.ajax({
            type: "POST",
            url: '<?= base_url('admin/offers_or_deals/getTotalDealItems') ?>',
            data: {},
            async: true,
            success: function (data) {
    ...
    $(document).on('click','#addItemButton',function() {
        ...
        if ($('#addItemForm').valid()) {
            ...
            if ((productIds.length > 0 || subProductIds.length > 0)) {
                ...
                var item_description = $('#addItemForm textarea[name="item_description"]').val();
